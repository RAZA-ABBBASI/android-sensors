package com.smarwey.sensorsexample;
    ...
    public final void onSensorChanged(SensorEvent event) {
        ...
        if( event.sensor.getType() == sensorInd){
            ...
            if(event.sensor.getType() ==  Sensor.TYPE_LIGHT){
                ...
                Log.d(TAG,"luminescence "+valueZ);
                if(valueZ > 40){
                    sensorLayout.setBackgroundColor(getResources().getColor(R.color.sensor_background_three));
                }else{
                    sensorLayout.setBackgroundColor(getResources().getColor(R.color.sensor_background_two));
                }
            }else if(event.sensor.getType() ==  Sensor.TYPE_PROXIMITY){
                //proximity sensor
                float distance = event.values[0];
                //Toast.makeText(this, "proximity "+distance,Toast.LENGTH_LONG).show();
                Log.d(TAG,"luminescence "+distance);
                if(distance > 25){
                    sensorLayout.setBackgroundColor(getResources().getColor(R.color.sensor_background_three));
                }else{
                    sensorLayout.setBackgroundColor(getResources().getColor(R.color.sensor_background_two));
                ...
                if(azimuth > 60 && azimuth < 90){
                    ...
                    sensorLayout.setBackgroundColor(getResources().getColor(R.color.sensor_background));
                }else{
                    sensorLayout.setBackgroundColor(getResources().getColor(R.color.sensor_background_four));
                }
            }
